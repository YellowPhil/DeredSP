...
std::string GetMimeType(const std::string& ext) {
	...
	std::wstring str;
	auto buffer_size = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, ext.c_str(), ext.size(), 0, 0);
	str.resize(buffer_size, '\0');
	MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, ext.c_str(), ext.size(), LPWSTR(str.c_str()), buffer_size);
	...
	if (SUCCEEDED(hr)) {
		...
		std::string narrow;
		auto size = WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK | WC_DEFAULTCHAR, strResult.c_str(), strResult.size(), 0, 0, 0, 0);
		narrow.resize(size, '\0');
		WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK | WC_DEFAULTCHAR, strResult.c_str(), strResult.size(), LPSTR(narrow.c_str()), size, "*", 0);
		return narrow;
	}
